    ...
    fn solve_part1(&self) -> i32 {
        let com_to_orbitors = {
            ...
            for relation in &self.orbital_relations {
                name_children.entry(&relation.com).or_insert_with(Vec::new).push(&relation.orbiter);
            }
            name_children
        };
        ...
        while !com_stack.is_empty() {
            ...
            let current_com_count = com_orbit_count.get(current_com).expect("no entry found").to_owned();
...
fn main() {
    let orbit_relations = read_input("/home/tim/projects/AoC19/resources/day6input").expect("unable to load input");
