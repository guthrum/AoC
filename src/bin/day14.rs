...
fn part2(input: &str) -> Option<(i64, i64)> {
    let total_ore: i64 = 1_000_000_000_000;

    // cost_per_f, made, ore remaining
    let mut last_trial = {
        let mut rg = RecipeGraph::try_from(input).unwrap();
        let ore_per_fuel = rg.required_ore_for_single_fuel()? as i64;
        (ore_per_fuel, 1, total_ore - ore_per_fuel)
    };
    // cost_per_f, made, ore remaining
    let mut best_trial = last_trial;
    let mut trial_count = 0;
    loop {
        // exit condition
        if last_trial.2 < 0 && (last_trial.1 - best_trial.1) == 1 {
            break;
        }
        trial_count += 1;

        let test_value = match last_trial.2.cmp(&0) {
            Ordering::Less => best_trial.1 + ((last_trial.1 - best_trial.1) / 2),
            _ => {
                let made_with_excess = last_trial.2 / last_trial.0;
                made_with_excess + last_trial.1
            }
        };
        let mut rg = RecipeGraph::try_from(input).unwrap();
        let ore_used = rg.require_ore_for_fuel(test_value as usize)? as i64;
        let ore_left = total_ore - ore_used;
        let ore_per_fuel = ore_used / test_value;

        last_trial = (ore_per_fuel, test_value, ore_left);
        if ore_left >= 0 && ore_left < best_trial.2 {
            best_trial = last_trial.clone();
        }
    }

    Some((trial_count, best_trial.1))
}
...
fn main() {
    ...
    println!("part 1 = {:?}", g.required_ore_for_single_fuel());
    println!(
        "part 2 = {:?}",
        part2(
            "5 HLJD, 1 QHSZD, 13 SKZX => 8 MQPH
