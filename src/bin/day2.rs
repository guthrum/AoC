...
fn run_machine(mut input: Vec<usize>) -> usize {
    ...
    while let Some(command) = parsed_command {
        ...
        program_counter += 4;
        parsed_command = parse_command(&input[program_counter .. std::cmp::min(program_counter+4, input.len())]);
    }
    0
}
...
fn main() {
    ...
    let mut noun = 0;
    let mut verb = 0;
    while execute(read_file("/home/tim/projects/AoC19/resources/day2input").expect("unable to load numbers"), noun, verb) != 19690720 {
        if noun == 99 {
            noun = 0;
            verb += 1;
        } else {
